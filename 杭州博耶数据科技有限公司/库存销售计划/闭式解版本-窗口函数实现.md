...
--       - 有实际销量: k=1, d=actual_qty
--       - 无实际销量: k=1-ratio/(181-N), d=0
--     仅情况1/2在1~180天使用闭式解; 情况3不依赖 cum_is_not_0_sum,k/d 不参与计算
-- ============================================================
closed_form_base AS (
    SELECT
        sc.*,
        ...
        CASE
            ...
            ELSE NULL
        END AS ratio,
        -- k(N): 递推系数(仅情况1/2在1~180天有效,其他场景k=1不影响计算)
        --   有实际销量 -> k=1 (R(N+1)=R(N)-actual_qty,直接减)
        --   无实际销量 -> k=1-ratio/(181-N) (R(N+1)=R(N)*(1-ratio/(181-N)))
        --   超周期/情况3 -> k=1 (不参与闭式解,置1避免影响累计乘积)
        CASE
            WHEN sc.lifecycle_day BETWEEN 1 AND 180
                 AND sc.sales_plan_tag IN ('<上架时间', '>=上架时间,<=180天')
                 AND COALESCE(sc.actual_qty, 0) = 0
        ...
        END AS k,
        -- d(N): 递推常数项(仅情况1/2在有实际销量时为 actual_qty,其他场景为0)
        CASE
            WHEN sc.lifecycle_day BETWEEN 1 AND 180
                 AND sc.sales_plan_tag IN ('<上架时间', '>=上架时间,<=180天')
                 AND COALESCE(sc.actual_qty, 0) > 0
...
-- ============================================================
-- 6.5 ★计算 plan_post_assist 和 cum_plan_qty
--     plan_post_assist 按 sales_plan_tag 分3种逻辑:
--       情况1: 始终=plan_post (实际销量始终为0)
--       情况2: 有实际取实际,无实际取plan_post
--       情况3: 1~179天取actual_qty, 第180天取Q-cum_actual(N), 超周期取(Q-cum_actual)*1
