...
> 以下为口径定义涉及的三张DWD表及其关键字段，便于后续引用时快速定位。
...
| 边界 | 若SKU从未产生销售，则 `first_sales_date` 为空 |

> **DWD层缺失说明**：361品牌的 `first_sales_date` 在 `dwd_feishu_product_all_d` 中为NULL，需在DWS层通过上述SQL从销售明细表计算并补充。这是已知的DWD层设计限制，不影响口径定义，但DWS层需额外处理。
...
> **361品牌说明**：361品牌无 `first_sales_date`，需先计算首次销售日期（3.5节），再计算已上架天数（3.6节），最后判断销售周期标签。销售周期标签为1~180天，业务逻辑中未展示完全的天数范围即为此三阶段划分。
...
| 口径 | 从首次销售日期到昨日，该SKU在所有渠道的实际销售金额总和 |
...
> **DWD层缺失说明**：当前DWD层无"二级市场补货"相关字段（`is_replenish`、`replenish_qty`在DWD-4韦德商品库中有但未保留到DWD-6统一商品库）。若需实现补货判断，需从 `dwd_feishu_product_wd_d` 补充这两个字段到统一商品库，或在DWS层单独关联韦德商品库。
...
| 上架时间 | product_all_d | shelf_date | 直接取值 |
| 首次销售日期 | product_all_d | first_sales_date | 韦德直接取值；361需从sales_all_d计算 |
...
| 可售周期(天) | 计算 | - | 在仓库存 / 日销量 |
| 日销量 | sales_all_d + 计算 | qty | 30天平均(核心渠道) |
| 系列日销 | sales_all_d + product_all_d + 计算 | qty, series | 同系列SUM(日销量) |
| 累计销量 | sales_all_d | qty | SUM(全渠道，首次销售~昨日) |
| 金额 | sales_all_d | amt | SUM(全渠道，首次销售~昨日) |
| 订货数量 | product_all_d | order_qty | 直接取值 |
| 达成比例 | 计算 | - | 累计销量 / 订货数量 |
| 昨日实际销售 | sales_all_d | qty | SUM(昨日，核心渠道) |
...
| 1~180天③实际销售 | sales_all_d | qty | SUM(第N天，全渠道) |
| 1~180天④达成情况 | 计算 | - | ③ / ② |
...
| 系列 | 直接取值 | 同SKU |
| 上架时间 | MIN(shelf_date) | SKC下取最早 |
| 首次销售日期 | MIN(first_sales_date) | SKC下取最早 |
| 已上架天数 | 基于SKC首次销售日期计算 | 独立计算 |
...
|----------|----------|----------|-------------|
| 361的 `first_sales_date` | 已上架天数、销售周期标签、1~180天分析 | 361 | DWS层从sales_all_d计算MIN(sales_date)补充 |
| 361的 `inventory_sku` | 在仓库存、可售周期 | 361 | 确认361是否有其他库存来源，或从品牌方库存表补充 |
| 二级市场补货标识 `is_replenish` | 达成比例分母调整 | 韦德 | 从dwd_feishu_product_wd_d补充到统一商品库，或DWS层单独关联 |
...
| 场景 | 口径A（业务文档暗示） | 口径B（本口径定义采用） |
|------|----------------------|------------------------|
| 日销量 | 仅核心渠道 | 仅核心渠道 |
| 累计销量 | 业务文档写"4个站销量总和" | **全部渠道**（更合理：累计应包含所有销售） |
| 1~180天实际销售 | 业务文档SUMIFS未限定渠道 | **全部渠道**（与累计销量一致） |

> 本口径定义采用口径B：日销量/昨日销售取核心渠道，累计销量/1~180天实际销售取全部渠道。若业务确认应统一为核心渠道，需调整。
...
> 本口径定义采用理解B，因为：1) 差异极小；2) SQL实现更简洁高效；3) 业务直觉更清晰（剩余量=总量-已实际卖出量）。若需严格按递推逻辑，需在DWS层用递归CTE或存储过程实现。
...
> 本口径定义按选项B设计（3.5节已给出361计算首次销售日期的逻辑），但需确认361业务上是否需要此分析。

#### 问题4：韦德 `order_qty` 的SKC/SSKU维度
...
> 当前DWD层注释写"统一为SKU维度订货量"，但韦德原始字段名含"sku"可能指"per SKU"也可能是"SKC维度的SKU订货量"。需与业务确认。
