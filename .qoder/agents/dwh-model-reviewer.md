...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `change_id` | string | ✅ | 关联的变更名称 |
| `spec_file_path` | string | ✅ | Spec 文件路径 |
| `target_tables` | string[] | ✅ | 待审查的表名列表 |
...
4. **业务规则落地**：spec 中的业务规则是否全部体现在 ETL / SQL / 约束中
5. **分层合规**：
...
- `data_warehouse_engineering_code_copilot/rules/modeling-standards.md` — 数仓建模与分层规范
- `data_warehouse_engineering_code_copilot/rules/sql-style.md` — SQL 编码规范（命名约定）
